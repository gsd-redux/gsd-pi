...
- Custom clients calling the built-in planning mutations must send a nonblank private `_meta["io.opengsd/idempotency-key"]` value and reuse it for retries. The key is transport metadata, not a public tool parameter; calls without it fail before mutation.
