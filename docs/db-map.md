...
`gsd_task_recovery_resume` appends a correction Work Checkpoint and `task.recovery.resumed` event for the exact current agent-owned abort after receiving a nonblank repair summary and non-empty structured evidence. The predecessor Attempt, its Result, the Recovery Action, and recovery budget remain unchanged. The event authorizes only the immediate lineage successor Attempt; stale or duplicate actions, open blockers, and actions superseded by a later Attempt fail closed. `gsd_task_reopen` records the same `task.recovery.resumed` event for every never-consumed agent-owned abort on the Task's prior Attempts, so a reopened `ready` Task carries no operative predecessor abort: dispatch no longer names a resume that would reject on the `ready` lifecycle, and the fresh lineage-linked Attempt claim carries current recovery authority.
