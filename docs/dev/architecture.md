...
| **OpenClaw** | [`integrations/openclaw/`](../../integrations/openclaw/) | Native OpenClaw plugin (`open-gsd-openclaw`, npm `@opengsd/open-gsd-openclaw`) — `/gsd` chat commands and per-conversation project binding backed by `gsd read --json`; supervised `gsd headless` runs and notifications follow in the next phase. See the package README. |
