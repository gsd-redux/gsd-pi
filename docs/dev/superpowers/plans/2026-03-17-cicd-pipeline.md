...
# ──────────────────────────────────────────────
# Stage 1: CI Builder
# Image: ghcr.io/gsd-redux/gsd-ci-builder
...
jobs:
  ...
  dev-publish:
    ...
    container:
      image: ghcr.io/gsd-redux/gsd-ci-builder:latest  # Pin to date tag after first build
    environment: dev
    outputs:
      ...
      - name: Build and push CI builder image
        run: |
          ...
          docker build --target builder \
            ...
            -t ghcr.io/gsd-redux/gsd-ci-builder:${{ steps.tag.outputs.date }} \
            .
          docker push ghcr.io/gsd-redux/gsd-ci-builder:latest
          docker push ghcr.io/gsd-redux/gsd-ci-builder:${{ steps.tag.outputs.date }}

      - name: Verify builder image
        run: |
          docker run --rm ghcr.io/gsd-redux/gsd-ci-builder:latest node --version
          docker run --rm ghcr.io/gsd-redux/gsd-ci-builder:latest rustc --version
```
...
4. **Enable GHCR:**
   - Ensure GitHub Container Registry is enabled for the `gsd-redux` org
