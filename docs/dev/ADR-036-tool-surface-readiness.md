...
5. **Workflow bridge warm-up (fail closed).** When workflow tools are enabled, the stdio CLI eagerly loads and shape-checks the executor and write-gate bridges before connecting (`warmWorkflowToolBridges`). A broken bridge fails the spawn with the actionable bridge error — visible in the SDK's MCP server status, where the runtime gate (2) turns it into a typed pre-turn failure. A healthy spawn pre-pays the bridge import, shrinking the race window.
...
- **Amended 2026-06-16:** the workflow surface is now required before Claude Code proceeds. The gate aborts when the workflow server is terminal, absent, or has not registered every required tool, including while `pending`, and routes the abort through `tool-unavailable` retry classification. A pending observation that already contains every required tool may proceed. This restores the pre-turn invariant that a Unit cannot begin while its required workflow MCP tools are unavailable.
- **Amended 2026-07-31:** A direct install of the independently published `@opengsd/mcp-server` does not contain the bundled GSD workflow runtime. When neither bridge is configured or co-located, the CLI omits workflow tools and serves only its bridge-independent session, read, and interactive surface. When a bridge is configured or discovered, warm-up remains fail-closed so an unloadable bridge never produces a dead advertised workflow surface.
