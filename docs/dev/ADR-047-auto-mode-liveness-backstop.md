...
"Every" includes the loop's own non-advancing exits, not just dispatch outcomes: the preflight stops (iteration ceiling, memory pressure, missing command context) are inside the adjudication boundary, and the idempotent "unit already active" skip is ledgered whenever no execution is actually in flight — a stale active-unit marker left behind by a thrown execution is otherwise a silent spin to the session limit (#1672). The skip stays exempt only while a unit really is running, where re-polling is the designed no-op.
