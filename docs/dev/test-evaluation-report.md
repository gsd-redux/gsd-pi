# Test Evaluation Report

Generated: 2026-06-20T21:30:18.157Z

## Summary

| Metric | Count |
|--------|------:|
| Source files | 1662 |
| Covered (named test) | 471 |
| Indirect (stem/other-name/suite tests) | 1191 |
| Untested | 0 |
| Unwired | 0 |
| Critical untested | 0 |
| High untested | 0 |

## Untested by area (top 20)

| Area | Untested / Total | Critical untested |
|------|----------------:|------------------:|
| pkg:contracts | 0 / 3 | 0 |
| pkg:daemon | 0 / 27 | 0 |
| pkg:gsd-agent-core | 0 / 33 | 0 |
| pkg:gsd-agent-modes | 0 / 94 | 0 |
| pkg:mcp-server | 0 / 26 | 0 |
| pkg:native | 0 / 31 | 0 |
| pkg:pi-agent-core | 0 / 32 | 0 |
| pkg:pi-ai | 0 / 76 | 0 |
| pkg:pi-coding-agent | 0 / 212 | 0 |
| pkg:pi-tui | 0 / 35 | 0 |
| pkg:rpc-client | 0 / 5 | 0 |
| scripts | 0 / 54 | 0 |
| src:app-paths.ts | 0 / 1 | 0 |
| src:bundled-extension-paths.ts | 0 / 1 | 0 |
| src:bundled-resource-path.ts | 0 / 1 | 0 |
| src:claude-cli-check.ts | 0 / 1 | 0 |
| src:cli-auto-routing.ts | 0 / 1 | 0 |
| src:cli-model-override.ts | 0 / 1 | 0 |
| src:cli-policy.ts | 0 / 1 | 0 |

## Priority gaps (critical/high untested)


Regenerate: `npm run audit:test-matrix -- --write-report`
