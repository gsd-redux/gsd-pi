...
Resolving or dismissing a genuine user/external Blocker does not turn the paused Attempt into a pass or resume that worker in place. GSD records a bounded agent-owned successor route, claims one fresh lineage-linked Attempt, and requires that Attempt to produce its own Result and current verification evidence before completion can publish.
