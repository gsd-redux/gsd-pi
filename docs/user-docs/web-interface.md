...
|----------|-------------|
| `GSD_WEB_AUTH_TOKEN` | Bearer token required on `/api/*` requests; generated by `gsd --web` unless auth is disabled |
| `GSD_WEB_HOST` | Web server host used for local origin checks; usually set by `gsd --web --host` |
| `GSD_WEB_PORT` | Web server port used for local origin checks; usually set by `gsd --web --port` |
| `GSD_WEB_ALLOWED_ORIGINS` | Comma-separated additional origins accepted by the bearer-token proxy |
