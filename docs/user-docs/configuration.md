...
- Custom clients calling `gsd_plan_milestone`, `gsd_plan_slice`, `gsd_plan_task`, `gsd_replan_slice`, `gsd_replan_task`, or `gsd_reassess_roadmap` must send a nonblank private `_meta["io.opengsd/idempotency-key"]` value and reuse it for retries. This is transport metadata, not a public tool parameter; missing identity fails before mutation.
