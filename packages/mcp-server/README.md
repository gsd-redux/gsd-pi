...
Planning mutations (`gsd_plan_milestone`, `gsd_plan_slice`, `gsd_plan_task`, `gsd_replan_slice`, `gsd_replan_task`, and `gsd_reassess_roadmap`) are atomic and replay-safe. A client that retries the same mutation across a new MCP request or server process should resend the same private request metadata value under `_meta["io.opengsd/idempotency-key"]`; this metadata is not a tool parameter and does not change the public tool schema or response. Canonical names and compatibility aliases resolve to the same operation identity.
