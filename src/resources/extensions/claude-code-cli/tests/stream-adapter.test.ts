...
describe("stream-adapter — workflow MCP readiness", () => {
	test("does not capture a source revision when workflow status is unavailable", async () => {
		const cwd = realpathSync(mkdtempSync(join(tmpdir(), "claude-sdk-no-workflow-")));
		const restore = setWorkflowMcpEnv({});
		let captures = 0;
		try {
			const stream = streamViaClaudeCode(
				{ id: "claude-sonnet-4-6" } as any,
				{ messages: [{ role: "user", content: "Answer without workflow tools." } as Message] },
				{
					cwd,
					_skipWorkflowMcpPreflightForTest: true,
					_captureMilestoneVerificationSourceRevisionForTest: () => {
						captures += 1;
						return { ok: true, sourceRevision: `sha256:${"a".repeat(64)}` };
					},
					async *_sdkQueryForTest() {
						yield {
							type: "result", subtype: "success", uuid: "result", session_id: "session",
							duration_ms: 1, duration_api_ms: 1, is_error: false, num_turns: 1,
							result: "done", stop_reason: "end_turn", total_cost_usd: 0,
