...
describe("stream-adapter — workflow MCP readiness", () => {
	...
	test("GSD phases do not capture source when milestone status is never called", async () => {
		const cwd = realpathSync(mkdtempSync(join(tmpdir(), "claude-sdk-gsd-no-status-call-")));
		const restore = setWorkflowMcpEnv({
			GSD_WORKFLOW_MCP_COMMAND: process.execPath,
			GSD_WORKFLOW_MCP_NAME: "gsd-workflow",
			GSD_WORKFLOW_MCP_ARGS: JSON.stringify(["-e", ""]),
		...
		let captures = 0;
		_setAutoActiveForTest(true);
		autoSession.currentUnit = { type: "plan-slice", id: "M001/S01", startedAt: 0, workspaceRoot: cwd } as never;
		try {
			const stream = streamViaClaudeCode(
				{ id: "claude-sonnet-4-6" } as any,
				{ messages: [{ role: "user", content: "Plan this slice." } as Message] },
				{
					cwd,
					_skipWorkflowMcpPreflightForTest: true,
					_captureMilestoneVerificationSourceRevisionForTest: () => {
					...
					async *_sdkQueryForTest() {
						yield {
							type: "result",
							subtype: "success",
							uuid: "result-gsd-no-status-call",
							session_id: "session-gsd-no-status-call",
							duration_ms: 1,
							duration_api_ms: 1,
							is_error: false,
							num_turns: 1,
