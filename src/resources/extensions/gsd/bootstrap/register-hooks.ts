...
import {
	maybePauseAutoForApprovalGate,
	resetPendingGatePauseGuard,
} from "./pending-gate-pause.js";
...
import {
	logWarning as safetyLogWarning,
...
async function loadWelcomeScreenModule(): Promise<
	WelcomeScreenModule | undefined
> {
	...
	for (const candidate of candidates) {
		try {
			...
			)) as Partial<WelcomeScreenModule>;
...
export function requestHasGsdCustomType(
	...
	return (requestCustomMessages ?? []).some(
		(message) =>
			...
			message.customType.startsWith("gsd-"),
	);
}
...
function autoBaseToolNamesForUnit(
	...
	for (const requested of requestedToolNames) {
		...
		for (const activeName of activeToolNames) {
			if (mcpToolMatchesBaseName(activeName, requested)) {
				...
				isWorkflowAliasTool(activeName) &&
				canonicalWorkflowToolName(activeName) === requested
			) {
...
export function buildMinimalGsdToolSet(
	activeToolNames: readonly string[],
): string[] {
	const preserved = activeToolNames.filter((name) => !isGsdManagedTool(name));
	const minimal = resolveScopedToolNames(
		activeToolNames,
		MINIMAL_GSD_TOOL_NAMES,
	);
...
export function buildMinimalAutoGsdToolSet(
	...
	const availableBaseTools = registeredToolNames.filter((name) =>
		autoBaseTools.has(name),
	);
	const preserved = [
		...new Set([
	...
	const result = withPreservedShimTools([
		...new Set([...preserved, ...scoped]),
	]);
	warnIfRequiredWorkflowToolsUnresolved(
		unitType,
		result,
...
function hasResolvedWorkflowTool(
	...
	return resolvedToolNames.some(
		(name) =>
			...
			mcpToolMatchesBaseName(name, requiredToolName),
	);
}
...
export function buildRunUatGsdToolSet(
	...
	const unresolved = RUN_UAT_WORKFLOW_TOOL_NAMES.filter(
		(tool) =>
			!resolved.some(
				(name) => name === tool || mcpToolMatchesBaseName(name, tool),
			),
	);
...
export function buildRequestScopedGsdToolSet(
	...
	for (
		let index = (requestCustomMessages?.length ?? 0) - 1;
		index >= 0;
		index--
	) {
		...
		if (
			...
			return buildMinimalGsdWorkflowToolSet(
				activeToolNames,
				registeredToolNames,
			);
		}
...
type GsdWorkflowScopeApi = Pick<
	ExtensionAPI,
	"getActiveTools" | "setActiveTools"
> &
	Partial<
		Pick<ExtensionAPI, "getAllTools" | "getVisibleSkills" | "setVisibleSkills">
	>;
...
function applyMinimalGsdToolSurface(pi: ExtensionAPI): void {
	...
	if (dash.active && dash.currentUnit) {
		...
		const registeredToolNames = resolveRegisteredToolNames(
			pi,
			currentToolNames,
		);
...
export function scopeGsdWorkflowToolsForDispatch(
	...
	const toolsChanged = !(
		scoped.length === current.length &&
		scoped.every((name, index) => name === current[index])
	);
	const canScopeSkills =
		unitHasSkillManifest(unitType) &&
		pi.getVisibleSkills &&
		pi.setVisibleSkills;
	...
	if (canScopeSkills && pi.setVisibleSkills) {
		applyUnitSkillVisibility(
			{ setVisibleSkills: pi.setVisibleSkills },
			unitType,
		);
	}
	return {
...
export function restoreGsdWorkflowTools(
	pi: Pick<ExtensionAPI, "setActiveTools"> &
		Partial<Pick<ExtensionAPI, "setVisibleSkills">>,
...
async function getDiscussionMilestoneIdFor(
	basePath: string,
): Promise<string | null> {
...
async function resetAskUserQuestionsTurnCache(): Promise<void> {
	const { resetAskUserQuestionsCache } = await import(
...
async function syncServiceTierStatus(ctx: ExtensionContext): Promise<void> {
	const { getEffectiveServiceTier, formatServiceTierFooterStatus } =
 ...
 */
function triggerCopilotCatalogSessionRefresh(
	...
	void (async () => {
		try {
			...
			if (resolveCopilotCatalogRefreshMode(prefs?.preferences) === "off")
				return;
			const notifyOnChanges = resolveCopilotCatalogNotifyOnChanges(
				prefs?.preferences,
			);
			...
			if (ctx.model?.provider === "github-copilot" && ctx.model.id) {
				const { maybeNotifyCheaperAlternative } = await import(
					"../copilot-catalog-notifications.js"
				);
 ...
 * same-tier alternative. `source: "set"` is deliberately excluded: automatic
 * routing calls pi.setModel(..., { persist: false }) and currently emits that
 * same source. Until the core event exposes explicit manual/dynamic origin,
 * only `cycle` is a provably manual selection event.
 */
function notifyOnManualCopilotModelSelection(
	...
	void (async () => {
		try {
			...
			const { getLastKnownCopilotCatalogSnapshot } = await import(
				"../copilot-catalog-session-refresh.js"
			);
			const { maybeNotifyCheaperAlternative } = await import(
				"../copilot-catalog-notifications.js"
			);
...
async function applyDisabledModelProviderPolicy(
	ctx: ExtensionContext,
): Promise<void> {
	try {
		const { resolveDisabledModelProvidersFromPreferences } = await import(
			"../preferences.js"
		);
		ctx.modelRegistry.setDisabledModelProviders(
 ...
 */
async function applyCompactionThresholdOverride(
	ctx: ExtensionContext,
): Promise<void> {
	try {
		const { loadEffectiveGSDPreferences } = await import("../preferences.js");
		...
		const raw =
			prefs?.preferences.context_management?.compaction_threshold_percent;
		const value =
			typeof raw === "number" &&
			Number.isFinite(raw) &&
			raw >= 0.5 &&
...
function isDestructiveConfirmationBlocking(basePath: string): boolean {
	return (
		deferredDestructiveConfirmationPauses.has(basePath) &&
...
function deferApprovalGateFromSnapshot(
	gateId: string,
	basePath: string,
...
function isGateResultPersistenceTool(toolName: string): boolean {
	return (
		toolName === "gsd_save_gate_result" || toolName === "gsd_uat_result_save"
	);
}
...
function isAbortedExecutionToolResult(
	toolName: string,
	result: unknown,
): boolean {
	...
	return (
		...
		details.force_resolved === true ||
		details.timed_out === true
	);
}

function isRetryableHarnessToolError(
	toolName: string,
	result: unknown,
	errorText: string,
): boolean {
...
function recordRetryableHarnessToolError(
	toolName: string,
	result: unknown,
	errorText: string,
): void {
	...
	recordCurrentUnitHarnessAbort({
		...
		reason:
			errorText ||
			"Tool execution failed before the unit could complete its gate evaluation.",
		toolName,
	});
}
...
function recordRetryableTurnAbort(event: {
	...
	const origin =
		typeof event.abortOrigin === "string" ? event.abortOrigin : undefined;
...
function beginSourceObservationStoreForCurrentUnit(ctx?: {
	...
	store.beginUnit({
		...
		basePath:
			dash.currentUnit.workspaceRoot ?? (dash.basePath || contextBasePath(ctx)),
	});
	return store;
}
...
function clearSourceObservationsAfterShell(canonicalName: string): void {
	if (!isAutoActive()) return;
	if (!isShellExecutionTool(canonicalName)) return;
	const dash = getAutoRuntimeSnapshot();
	if (
		!dash.currentUnit ||
		!supportsSourceObservationsForUnit(dash.currentUnit.type)
	)
		return;
...
function extractGateQuestionId(input: unknown): string | undefined {
	const questions: Array<{ id?: unknown }> =
		((input as { questions?: unknown })?.questions as Array<{
			id?: unknown;
		}>) ?? [];
...
function isApprovalGateBlocking(basePath: string): boolean {
	return (
		Boolean(getPendingGate(basePath)) || deferredApprovalGates.has(basePath)
	);
}
...
function isContextDraftSummarySave(toolName: string, input: unknown): boolean {
	...
	return (
		(input as { artifact_type?: unknown }).artifact_type === "CONTEXT-DRAFT"
	);
}
...
function normalizeAskUserQuestionsAnswer(answer: any): any {
	if (!answer || typeof answer !== "object" || answer.selected !== undefined)
		return answer;
	return answer.answers !== undefined
		? { ...answer, selected: answer.answers }
		: answer;
}
...
function normalizeAskUserQuestionsRound(value: unknown): any | null {
	...
	if (
		!response &&
		round.cancelled === undefined &&
		round.timed_out === undefined
	)
		return null;
		...
		response: {
			...
			answers: Object.fromEntries(
				Object.entries(response.answers ?? {}).map(([id, answer]) => [
					id,
					normalizeAskUserQuestionsAnswer(answer),
				]),
			),
		},
	};
...
function resolveAskUserQuestionsGateDetails(event: {
	...
	const structured = (
		event.result as { structuredContent?: unknown } | undefined
	)?.structuredContent;
...
async function ensureMilestoneShell(
	basePath: string,
	milestoneId: string,
): Promise<string> {
	...
	const isLegacyLayout =
		...
		(() => {
			try {
				return readdirSync(legacy).some((e) =>
					statSync(join(legacy, e)).isDirectory(),
				);
			} catch {
				return false;
			}
	...
	const milestoneDir =
		resolveMilestonePath(basePath, milestoneId) ??
	...
	try {
		...
		safetyLogWarning(
			"guided",
			`failed to persist milestone shell for ${milestoneId}: ${(err as Error).message}`,
		);
	}
...
async function saveDiscussionQuestionRound(
	...
	const isLegacyDir =
		milestoneDir.startsWith(legacyBase + "/") ||
		milestoneDir.startsWith(legacyBase + "\\");
	...
	try {
		...
		if (await ensureDbOpen(basePath)) {
			...
			insertArtifact({
				path: relMilestoneFile(basePath, milestoneId, "CONTEXT-DRAFT").replace(
					/^\.gsd\//,
					"",
				),
		...
		safetyLogWarning(
			"guided",
			`failed to persist CONTEXT-DRAFT artifact for ${milestoneId}: ${(err as Error).message}`,
		);
	}
}

function withDepthGateDisplayReason<
	T extends { block: boolean; reason?: string },
>(
	result: T,
...
function initSessionNotifications(ctx: ExtensionContext): void {
	...
	notifyPreferenceDiagnostics(ctx, contextBasePath(ctx), {
...
export function registerHooks(
	...
	void import("../provider-switch-observer.js").then((m) =>
		m.installProviderSwitchObserver(),
	);
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		try {
			...
			if (!isInAutoWorktree(basePath)) {
				const { needsFlatPhaseMigration } = await import(
					"../flat-phase-migration.js"
				);
				if (needsFlatPhaseMigration(basePath)) {
					const { ensureDbOpen } = await import("./dynamic-tools.js");
					const opened = await ensureDbOpen(basePath);
					if (opened) {
						const { migrateToFlatPhase } = await import(
							"../flat-phase-migration.js"
						);
		...
		try {
			...
			const { pruneStaleFlatPhaseBackups } = await import(
				"../flat-phase-migration.js"
			);
			...
			safetyLogWarning(
				"bootstrap",
				`flat-phase backup pruning: ${err instanceof Error ? err.message : String(err)}`,
			);
		}
		...
		try {
			const { loadEffectiveGSDPreferences } = await import("../preferences.js");
			const prefs = loadEffectiveGSDPreferences(basePath);
			process.env.GSD_SHOW_TOKEN_COST = prefs?.preferences.show_token_cost
				? "1"
				: "";
		} catch {
			/* non-fatal */
		...
		if (!isAutoActive() && !isAutoPaused() && !preserveCloseoutSurface) {
			try {
				const { reenterActiveWorktreeIfNeeded } = await import(
					"../worktree-reentry.js"
				);
	...
	pi.on("before_agent_start", async (event, ctx: ExtensionContext) => {
		...
		if (
			pendingApprovalGate &&
			isExplicitApprovalResponse(event.prompt, pendingApprovalGate)
		) {
			...
			hostWriteGateAdapter.markApprovalGateVerified(
				pendingApprovalGate,
				beforeAgentBasePath,
			);
			const milestoneId =
				extractDepthVerificationMilestoneId(pendingApprovalGate);
			if (milestoneId)
				hostWriteGateAdapter.markDepthVerified(
					milestoneId,
					beforeAgentBasePath,
				);
			hostWriteGateAdapter.clearPending(beforeAgentBasePath);
			if (isAutoPaused() && !isAutoActive()) {
				const { resumeAutoAfterProviderDelay } = await import(
					"./provider-error-resume.js"
				);
				void resumeAutoAfterProviderDelay(pi, ctx).catch((err) => {
					const message = err instanceof Error ? err.message : String(err);
					ctx.ui.notify(
						`Failed to resume auto-mode after approval: ${message}`,
						"warning",
					);
				});
			}
		...
		} = await import("../skill-discovery.js");
		if (hasSkillSnapshot()) {
			const loadedSkills = await refreshCatalogForNewSkills({
				reload: () =>
					(ctx as ExtensionContext & { reload: () => Promise<void> }).reload(),
				notify: (message, level) => ctx.ui.notify(message, level),
			});
			if (loadedSkills.length > 0) {
				systemPrompt = appendDiscoveredSkillsFallback(
					ctx.getSystemPrompt(),
					loadedSkills,
				);
			}
		...
		const gsdResult = await buildBeforeAgentStartResult(
			{ ...event, systemPrompt },
			ctx,
		);
	...
	pi.on("agent_end", async (event, ctx: ExtensionContext) => {
		...
		try {
			...
			const destructiveConfirmationBlocking =
				isDestructiveConfirmationBlocking(agentEndBasePath);
	...
	pi.on("message_end", async (event) => {
		const { suppressTerminalDeletedWorktreeMessageEnd } = await import(
			"./agent-end-recovery.js"
		);
		suppressTerminalDeletedWorktreeMessageEnd(event);
		if (isAutoActive()) {
			const { sanitizePrematureCloseoutMessageEnd } = await import(
				"../auto-closeout-messaging.js"
			);
	...
	pi.on("session_before_compact", async (event, ctx) => {
		...
		const { writeContextModeCompactionSnapshot } = await import(
			"../context-mode-snapshot.js"
		);
	...
	pi.on("message_update", async (event, ctx: ExtensionContext) => {
		...
		if (!unitType) {
			try {
				const { getPendingDeepProjectSetupUnitForContext } = await import(
					"../guided-flow.js"
				);
				const pending = getPendingDeepProjectSetupUnitForContext(
					ctx,
					contextBasePath(ctx),
				);
		...
		if (gateId) {
			...
			if (
				gateMilestoneId &&
				isMilestoneDepthVerifiedInSnapshot(gateSnapshot, gateMilestoneId)
			)
				return;
	...
	pi.on("session_shutdown", async (_event, ctx: ExtensionContext) => {
		const { isParallelActive, shutdownParallel } = await import(
	...
	pi.on("tool_call", async (event, ctx) => {
		...
		if (loopCheck.block) {
			recordCurrentUnitHarnessAbort({
				...
				reason:
					loopCheck.reason ??
					"Tool-call loop guard blocked a repeated tool call.",
				toolName,
				count: loopCheck.count,
			});
			return {
				block: true,
				reason: formatLoopGuardBlockReason(loopCheck.reason),
			};
		}
		...
		if (activeUnitType) {
			...
			if (isToolCallEventType("write", event)) {
				...
				agentClasses = extractSubagentAgentClasses(
			...
			const planningGuard = shouldBlockPlanningUnit(
				...
				planningBasePath,
				activeUnitType,
				resolveEffectivePlanningToolsPolicy(
					activeUnitType,
					manifest?.tools,
					planningBasePath,
				),
		...
		if (isToolCallEventType("bash", event)) {
			...
			if (classification.destructive) {
				...
				if (consumeDestructiveConfirmation(command, guardBasePath)) {
					safetyLogWarning(
						"safety",
						`destructive command confirmed: ${classification.labels.join(", ")}`,
						{
	...
	pi.on("tool_result", async (event, ctx) => {
		...
		if (!event.isError && !isAutoActive() && isUnitCloseoutTool(toolName)) {
			try {
				runInteractiveUnitCloseout({
					basePath: resolveWorkflowToolBasePath(
						ctx,
						event.input as { milestone_id?: string },
					),
				...
				safetyLogWarning(
					"engine",
					`interactive unit closeout failed: ${err instanceof Error ? err.message : String(err)}`,
				);
			}
		}
		...
		const questions: any[] =
			(event.input as any)?.questions ?? details?.questions ?? [];
		...
		if (gateResult.status === "waiting") {
			...
			return {
				content: [
					{
						type: "text" as const,
						text: formatPendingAskUserQuestionsGateMessage(
							gateResult.pendingGateId,
							gateResult.interrupted,
						),
					},
				],
		...
		if (gateResult.status === "timeout") {
			...
			return {
				content: [
					{
						type: "text" as const,
						text: formatTimedOutAskUserQuestionsGateMessage(
							gateResult.pendingGateId,
						),
					},
				],
		...
		const roundOutcome = evaluateAskUserQuestionsRound(
			questions,
			details ?? {},
		);
		...
		await saveDiscussionQuestionRound(
			basePath,
			milestoneId,
			questions,
			details,
	...
	pi.on("tool_execution_start", async (event, ctx) => {
		...
		if (execDash.basePath && execDash.currentUnit?.type === "execute-task") {
			const {
				...
				slice: xSid,
				task: xTid,
			} = parseUnitId(execDash.currentUnit.id);
			if (xMid && xSid && xTid) {
		...
		if (isAutoActive()) {
			...
			if (dash.basePath && dash.currentUnit?.type === "execute-task") {
				const {
					...
					slice: pSid,
					task: pTid,
				} = parseUnitId(dash.currentUnit.id);
				if (pMid && pSid && pTid) {
	...
	pi.on("adjust_tool_set", async (event) => {
		...
		const compatible = event.activeToolNames.filter(
		...
		const requestRegisteredToolNames =
			guidedUnit?.unitType === "run-uat"
				? compatibleRegisteredToolNames
				: registeredToolNames;
		const requestScoped = buildRequestScopedGsdToolSet(
			guidedUnit?.unitType === "run-uat"
				? aliasFilteredCompatible
				: providerCompatible,
