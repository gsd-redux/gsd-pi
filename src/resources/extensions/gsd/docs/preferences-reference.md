...
- `workspace`: optional parent-workspace repository registry. Keys:
  - `mode`: `"project"` or `"parent"` — registry mode. Default: `"project"`.
  - `repositories`: object keyed by repository id. Each repository supports:
    ...
    - `commit_policy`: `"auto"` or `"skip"` — optional turn-commit policy for auto-mode commit actions. `"skip"` suppresses commit execution for that target repo.
