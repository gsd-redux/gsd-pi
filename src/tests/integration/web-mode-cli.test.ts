...
test('package hooks declare a concrete staged web host', () => {
  ...
  assert.equal(rootPackage.scripts['gsd:web'], 'npm run build:upstream-pi && npm run copy-resources && node scripts/build-web-if-stale.cjs && node scripts/dev-cli.js --web')
